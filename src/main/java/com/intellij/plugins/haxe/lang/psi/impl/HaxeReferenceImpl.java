...
abstract public class HaxeReferenceImpl extends HaxeExpressionImpl implements HaxeReference {
    ...
    for (PsiElement element : nameList) {
      ...
      if (element instanceof HaxeComponentName) {
        ...
        if (null != parent && parent.isValid()) {
          ...
          // element;
          if (!HaxeJavaUtil.isPsiPackage(parent)) {
            elementToAdd = parent;
          }
        }
      }
  ...
  private HaxeResolveResult _resolveHaxeClassInternal() {
    ...
    if (resolve != null) {
      ...
      if (HaxeJavaUtil.isPsiPackage(resolve)) {
  ...
  public PsiElement bindToElement(@NotNull PsiElement element) throws IncorrectOperationException {
    ...
    else if (HaxeJavaUtil.isPsiPackage(element)) {
      bindToPackage(element);
    }
  ...
  private void bindToPackage(PsiElement element) {
    String qualifiedName = HaxeJavaUtil.getPackageQualifiedName(element);
    if (qualifiedName == null) return;
    final HaxeImportStatement importStatement =
      HaxeElementGenerator.createImportStatementFromPath(getProject(), qualifiedName);
  ...
  public Object[] getVariants() {
    ...
    com.intellij.psi.PsiPackage[] leftSubPackages = HaxeJavaUtil.getSubPackages(leftTarget);
    if (leftSubPackages != null) {
      variants.addAll(HaxePackageLookupElement.convert(leftSubPackages));
    }
      ...
      com.intellij.psi.PsiPackage[] rootSubPackages = HaxeJavaUtil.getSubPackages(HaxeJavaUtil.findPackage(getElement().getProject(), ""));
      if (rootSubPackages != null) variants.addAll(HaxePackageLookupElement.convert(rootSubPackages));
    }
  ...
  private static ResolveResult[] toCandidateInfoArray(List<? extends PsiElement> elements) {
    ...
    final List<ResolveResult> result = new java.util.ArrayList<>(elements.size());
